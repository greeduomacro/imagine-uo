...
{
	public class LowerRegCostRune : BaseMinorRune
	{
		[Constructable]
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060434, this.BaseAmount.ToString() ); // lower reagent cost ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.LowerRegCost + value <= max )
								i.Attributes.LowerRegCost += value;
							else
								i.Attributes.LowerRegCost = max;
						}
