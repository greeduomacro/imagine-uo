...
{
	public class CastSpeedRune : BaseMinorRune
	{
		[Constructable]
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060413, this.BaseAmount.ToString() ); // faster casting ~1_val~
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.CastSpeed + value <= max )
								i.Attributes.CastSpeed += value;
							else
								i.Attributes.CastSpeed = max;
						}
