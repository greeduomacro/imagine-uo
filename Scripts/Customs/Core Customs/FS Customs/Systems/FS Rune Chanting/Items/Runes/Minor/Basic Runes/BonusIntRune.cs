...
{
	public class BonusIntRune : BaseMinorRune
	{
		[Constructable]
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060432, this.BaseAmount.ToString() ); // intelligence bonus ~1_val~
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.BonusInt + value <= max )
								i.Attributes.BonusInt += value;
							else
								i.Attributes.BonusInt = max;
						}
