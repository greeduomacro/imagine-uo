...
{
	public class RegenStamRune : BaseMinorRune
	{
		[Constructable]
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060443, this.BaseAmount.ToString() ); // stamina regeneration ~1_val~
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.RegenStam + value <= max )
								i.Attributes.RegenStam += value;
							else
								i.Attributes.RegenStam = max;
						}
