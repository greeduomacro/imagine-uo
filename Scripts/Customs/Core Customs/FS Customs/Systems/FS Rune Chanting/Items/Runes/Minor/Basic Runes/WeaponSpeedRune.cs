...
{
	public class WeaponSpeedRune : BaseMinorRune
	{
		[Constructable]
		public WeaponSpeedRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 30;
		}

		public WeaponSpeedRune( Serial serial ) : base( serial )
		{
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060486, this.BaseAmount.ToString() ); // swing speed increase ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.WeaponSpeed + value <= max )
								i.Attributes.WeaponSpeed += value;
							else
								i.Attributes.WeaponSpeed = max;
						}
