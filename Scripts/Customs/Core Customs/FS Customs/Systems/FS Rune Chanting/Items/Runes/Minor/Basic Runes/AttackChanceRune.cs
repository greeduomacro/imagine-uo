...
{
	public class AttackChanceRune : BaseMinorRune
	{
		[Constructable]
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060415, this.BaseAmount.ToString() ); // hit chance increase ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.AttackChance + value <= max )
								i.Attributes.AttackChance += value;
							else
								i.Attributes.AttackChance = max;
						}
