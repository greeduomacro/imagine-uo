...
{
	public class SpellDamageRune : BaseMinorRune
	{
		[Constructable]
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060483, this.BaseAmount.ToString() ); // spell damage increase ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.SpellDamage + value <= max )
								i.Attributes.SpellDamage += value;
							else
								i.Attributes.SpellDamage = max;
						}
