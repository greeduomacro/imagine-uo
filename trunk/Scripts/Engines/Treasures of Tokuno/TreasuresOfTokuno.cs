	...
	public class TreasuresOfTokuno
	{
		...
		private static Type[] m_LesserArtifactsTotal = new Type[]
			{
				...
				typeof( AncientUrn ), 
				typeof( HonorableSwords ), 
				typeof( PigmentsOfTokuno ), 
				typeof( FluteOfRenewal ),
				typeof( LeurociansMempoOfFortune ), 
				typeof( LesserPigmentsOfTokuno ), 
				typeof( MetalPigmentsOfTokuno ), 
				typeof( LeurociansMempoOfFortune ),
		...
		public static void HandleKill( Mobile victim, Mobile killer )
		{
			...
			if( chance > Utility.RandomDouble() )
			{
				...
				try
				{
					i = Activator.CreateInstance( m_LesserArtifactsTotal[Utility.Random( m_LesserArtifactsTotal.Length )] ) as Item;
