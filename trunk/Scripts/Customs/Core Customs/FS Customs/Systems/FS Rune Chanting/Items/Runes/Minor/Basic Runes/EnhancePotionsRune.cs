...
{
	public class EnhancePotionsRune : BaseMinorRune
	{
		[Constructable]
		public EnhancePotionsRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 25;
		}

		public EnhancePotionsRune( Serial serial ) : base( serial )
		{
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060411, this.BaseAmount.ToString() ); // enhance potions ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.EnhancePotions + value <= max )
								i.Attributes.EnhancePotions += value;
							else
								i.Attributes.EnhancePotions = max;
						}
