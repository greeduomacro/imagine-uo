...
{
	public class PhysicalBonusRune : BaseMinorRune
	{
		[Constructable]
		public PhysicalBonusRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 15;
		}

		public PhysicalBonusRune( Serial serial ) : base( serial )
		{
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060448, this.BaseAmount.ToString() ); // physical resist ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseJewel || item is BaseHat )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.PhysicalBonus + value <= max )
								i.PhysicalBonus += value;
							else
								i.PhysicalBonus = max;
						}
						...
						if ( item is BaseWeapon )
						{
							...
							if ( i.WeaponAttributes.ResistPhysicalBonus + value <= max )
								i.WeaponAttributes.ResistPhysicalBonus += value;
							else
								i.WeaponAttributes.ResistPhysicalBonus = max;
						}
						...
						if ( item is BaseJewel )
						{
							...
							if ( i.Resistances.Physical + value <= max )
								i.Resistances.Physical += value;
							else
								i.Resistances.Physical = max;
						}
