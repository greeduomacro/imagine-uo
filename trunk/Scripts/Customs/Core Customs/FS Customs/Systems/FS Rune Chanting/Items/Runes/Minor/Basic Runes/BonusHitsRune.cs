...
{
	public class BonusHitsRune : BaseMinorRune
	{
		[Constructable]
		public BonusHitsRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 5;
		}

		public BonusHitsRune( Serial serial ) : base( serial )
		{
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060431, this.BaseAmount.ToString() ); // hit point increase ~1_val~
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.BonusHits + value <= max )
								i.Attributes.BonusHits += value;
							else
								i.Attributes.BonusHits = max;
						}
