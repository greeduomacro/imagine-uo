...
{
	public class HitLeechStamRune : BaseMinorRune
	{
		[Constructable]
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060430, this.BaseAmount.ToString() ); // hit stamina leech ~1_val~%
		}
				...
				if ( item is BaseWeapon )
				{
					...
					else
					{
						...
						if ( item is BaseWeapon )
						{
							...
							if ( i.WeaponAttributes.HitLeechStam + value <= max )
								i.WeaponAttributes.HitLeechStam += value;
							else
								i.WeaponAttributes.HitLeechStam = max;
						}
