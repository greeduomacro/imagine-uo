...
{
	public class ReflectPhysicalRune : BaseMinorRune
	{
		[Constructable]
		public ReflectPhysicalRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 15;
		}

		public ReflectPhysicalRune( Serial serial ) : base( serial )
		{
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060442, this.BaseAmount.ToString() ); // reflect physical damage ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.ReflectPhysical + value <= max )
								i.Attributes.ReflectPhysical += value;
							else
								i.Attributes.ReflectPhysical = max;
						}
