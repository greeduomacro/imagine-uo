...
{
	public class HitLeechManaRune : BaseMinorRune
	{
		[Constructable]
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060427, this.BaseAmount.ToString() ); // hit mana leech ~1_val~%
		}
				...
				if ( item is BaseWeapon )
				{
					...
					else
					{
						...
						if ( item is BaseWeapon )
						{
							...
							if ( i.WeaponAttributes.HitLeechMana + value <= max )
								i.WeaponAttributes.HitLeechMana += value;
							else
								i.WeaponAttributes.HitLeechMana = max;
						}
