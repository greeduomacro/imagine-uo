...
{
	public class ColdBonusRune : BaseMinorRune
	{
		[Constructable]
		public ColdBonusRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 15;
		}

		public ColdBonusRune( Serial serial ) : base( serial )
		{
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060445, this.BaseAmount.ToString() ); // cold resist ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseJewel || item is BaseHat )
				{
					...
					else
					{
						...
						if ( item is BaseWeapon )
						{
							...
							if ( i.WeaponAttributes.ResistColdBonus + value <= max )
								i.WeaponAttributes.ResistColdBonus += value;
							else
								i.WeaponAttributes.ResistColdBonus = max;
						}
