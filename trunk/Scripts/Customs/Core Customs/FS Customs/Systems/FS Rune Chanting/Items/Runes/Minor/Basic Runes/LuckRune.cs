...
{
	public class LuckRune : BaseMinorRune
	{
		[Constructable]
		public LuckRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 100;
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060436, this.BaseAmount.ToString() ); // luck ~1_val~
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.Luck + value <= max )
								i.Attributes.Luck += value;
							else
								i.Attributes.Luck = max;
						}
