...
{
	public class DurabilityBonusRune : BaseMinorRune
	{
		[Constructable]
		public DurabilityBonusRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 100;
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060410, this.BaseAmount.ToString() ); // durability ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseHat || item is BaseWeapon )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.ArmorAttributes.DurabilityBonus + value <= max )
								i.ArmorAttributes.DurabilityBonus += value;
							else
								i.ArmorAttributes.DurabilityBonus = max;
						}
						...
						if ( item is BaseHat )
						{
							...
							if ( i.ClothingAttributes.DurabilityBonus + value <= max )
								i.ClothingAttributes.DurabilityBonus += value;
							else
								i.ClothingAttributes.DurabilityBonus = max;
						}
						...
						if ( item is BaseWeapon )
						{
							...
							if ( i.WeaponAttributes.DurabilityBonus + value <= max )
								i.WeaponAttributes.DurabilityBonus += value;
							else
								i.WeaponAttributes.DurabilityBonus = max;
						}
