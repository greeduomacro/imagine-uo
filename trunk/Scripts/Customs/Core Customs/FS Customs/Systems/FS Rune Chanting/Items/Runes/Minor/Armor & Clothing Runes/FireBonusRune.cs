...
{
	public class FireBonusRune : BaseMinorRune
	{
		[Constructable]
		public FireBonusRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 15;
		}

		public FireBonusRune( Serial serial ) : base( serial )
		{
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060447, this.BaseAmount.ToString() ); // fire resist ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseJewel || item is BaseHat )
				{
					...
					else
					{
						...
						if ( item is BaseWeapon )
						{
							...
							if ( i.WeaponAttributes.ResistFireBonus + value <= max )
								i.WeaponAttributes.ResistFireBonus += value;
							else
								i.WeaponAttributes.ResistFireBonus = max;
						}
						...
						if ( item is BaseJewel )
						{
							...
							if ( i.Resistances.Fire + value <= max )
								i.Resistances.Fire += value;
							else
								i.Resistances.Fire = max;
						}
