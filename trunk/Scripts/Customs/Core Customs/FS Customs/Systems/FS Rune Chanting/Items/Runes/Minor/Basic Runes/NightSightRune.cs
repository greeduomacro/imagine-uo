...
{
	public class NightSightRune : BaseMinorRune
	{
		[Constructable]
		public NightSightRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 1;
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.NightSight + value <= max )
								i.Attributes.NightSight += value;
							else
								i.Attributes.NightSight = max;
						}
