...
{
	public class WeaponDamageRune : BaseMinorRune
	{
		[Constructable]
		public WeaponDamageRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 50;
		}

		public WeaponDamageRune( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.SendLocalizedMessage( 1060401 ); // This item must be in your backpack to be used.
			}
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060483, this.BaseAmount.ToString() ); // damage increase ~1_val~%
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.WeaponDamage + value <= max )
								i.Attributes.WeaponDamage += value;
							else
								i.Attributes.WeaponDamage = max;
						}
