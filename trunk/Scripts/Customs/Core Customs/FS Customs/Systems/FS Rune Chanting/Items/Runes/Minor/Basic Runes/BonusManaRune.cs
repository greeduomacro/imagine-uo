...
{
	public class BonusManaRune : BaseMinorRune
	{
		[Constructable]
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060439, this.BaseAmount.ToString() ); // mana increase ~1_val~
		}
				...
				if ( item is BaseArmor || item is BaseWeapon || item is BaseHat || item is BaseJewel || item is BaseTalisman || item is BaseQuiver || item is Spellbook )
				{
					...
					else
					{
						...
						if ( item is BaseArmor )
						{
							...
							if ( i.Attributes.BonusMana + value <= max )
								i.Attributes.BonusMana += value;
							else
								i.Attributes.BonusMana = max;
						}
