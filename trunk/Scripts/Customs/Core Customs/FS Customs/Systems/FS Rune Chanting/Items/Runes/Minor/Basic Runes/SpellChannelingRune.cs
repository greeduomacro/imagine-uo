...
{
	public class SpellChannelingRune : BaseMinorRune
	{
		[Constructable]
		public SpellChannelingRune() : base()
		{
			BaseAmount = 1;
			MaxAmount = 1;
		}

		public SpellChannelingRune( Serial serial ) : base( serial )
		{
		}
				...
				if ( item is BaseShield || item is BaseWeapon )
				{
					...
					else
					{
						...
						if ( item is BaseShield )
						{
							...
							if ( i.Attributes.SpellChanneling + value <= max )
								i.Attributes.SpellChanneling += value;
							else
								i.Attributes.SpellChanneling = max;
						}
